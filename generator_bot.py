...
msg_ids = {"live": None, "panel": None, "web": None}

def save():
    with open(os.path.expanduser("~/gen_state.json"), "w") as f:
        json.dump(gen, f)
...
def web_embed():
    ...
    e.add_field(name="🔗 URL", value="`http://localhost:8080/generator31.html`", inline=False)
...
async def update():
    ...
    for key, ch_id, embed, view in jobs:
        ...
        try:
            ...
            msg_ids[key] = None; print(f"Gen {key}: {ex}")
